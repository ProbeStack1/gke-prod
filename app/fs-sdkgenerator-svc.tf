resource "kubernetes_deployment_v1" "fs_sdkgenerator_svc" {
  metadata {
    name      = "fs-sdkgenerator-svc"
    namespace = "forgestudio-prod"

    labels = {
      app = "fs-sdkgenerator-svc"
    }
  }
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name  = "fs-sdkgenerator-svc"
          image = var.fs_sdkgenerator_svc_image
          ...
          readiness_probe {
            http_get {
              path = "/api/v1/sdk/actuator/health"
              port = 8080
            }
...
resource "kubectl_manifest" "fs_sdkgenerator_backend" {
  yaml_body = <<YAML
apiVersion: cloud.google.com/v1
kind: BackendConfig
metadata:
  name: fs-sdkgenerator-backend
  namespace: forgestudio-prod
spec:
  healthCheck:
    requestPath: /api/v1/sdk/actuator/health
    port: 8080
    type: HTTP
YAML
}

resource "kubernetes_service_v1" "fs_sdkgenerator_svc" {
  metadata {
    name      = "fs-sdkgenerator-svc"
    namespace = "forgestudio-prod"
    ...
    annotations = {
      ...
      "cloud.google.com/backend-config" = jsonencode({
        default = "fs-sdkgenerator-backend"
      })
    }
