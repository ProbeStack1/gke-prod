resource "kubernetes_deployment_v1" "fq_documentation_mgmt_svc" {
  metadata {
    name      = "fq-documentation-mgmt-svc"
    namespace = "forgeq-prod"
    labels = {
      app = "fq-documentation-mgmt-svc"
    }
  }
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name  = "fq-documentation-mgmt-svc"
          image = var.fq_documentation_mgmt_svc_image
          ...
          readiness_probe {
            http_get {
              path = "/api/v1/document/actuator/health"
              port = 8080
            }
...
resource "kubectl_manifest" "fq_documentation_mgmt_backend" {
  yaml_body = <<YAML
apiVersion: cloud.google.com/v1
kind: BackendConfig
metadata:
  name: fq-documentation-mgmt-backend
  namespace: forgeq-prod
spec:
  healthCheck:
    requestPath: /api/v1/document/actuator/health
    port: 8080
    type: HTTP
YAML
}

resource "kubernetes_service_v1" "fq_documentation_mgmt_svc" {
  metadata {
    name      = "fq-documentation-mgmt-svc"
    namespace = "forgeq-prod"
    ...
    annotations = {
      ...
      "cloud.google.com/backend-config" = jsonencode({
        default = "fq-documentation-mgmt-backend"
      })
    }
