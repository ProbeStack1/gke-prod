resource "kubernetes_deployment_v1" "fq_functional_test_mgmt_svc" {
  metadata {
    name      = "fq-functional-test-mgmt-svc"
    namespace = "forgeq-prod"
    labels = {
      app = "fq-functional-test-mgmt-svc"
    }
  }
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name  = "fq-functional-test-mgmt-svc"
          image = var.fq_functional_test_mgmt_svc_image
          ...
          readiness_probe {
            http_get {
              path = "/functional-tests/actuator/health"
              port = 8080
            }
...
spec:
  healthCheck:
    requestPath: /functional-tests/actuator/health
    port: 8080
    type: HTTP
YAML
}

resource "kubernetes_service_v1" "fq_functional_test_mgmt_svc" {
  metadata {
    name      = "fq-functional-test-mgmt-svc"
    namespace = "forgeq-prod"
