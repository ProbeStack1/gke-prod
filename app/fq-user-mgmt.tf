resource "kubernetes_deployment_v1" "fq_user_mgmt_svc" {
  metadata {
    name      = "fq-user-mgmt-svc"
    namespace = "forgeq-prod"

    labels = {
      app = "fq-user-mgmt-svc"
    }
  }
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name  = "fq-user-mgmt-svc"
          image = var.fq_user_mgmt_svc_image
...
resource "kubectl_manifest" "fq_user_mgmt_backend" {
  yaml_body = <<YAML
apiVersion: cloud.google.com/v1
kind: BackendConfig
metadata:
  name: fq-user-mgmt-backend
  namespace: forgeq-prod
spec:
  healthCheck:
...
resource "kubernetes_service_v1" "fq_user_mgmt_svc" {
  metadata {
    name      = "fq-user-mgmt-svc"
    namespace = "forgeq-prod"
