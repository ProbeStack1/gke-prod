...
resource "kubectl_manifest" "fsp_test_generation_backend" {
  yaml_body = <<YAML
apiVersion: cloud.google.com/v1
kind: BackendConfig
metadata:
  name: fsp-test-generation-backend
  namespace: forgesphere-prod
spec:
  healthCheck:
    requestPath: /test/actuator/health
    port: 8080
    type: HTTP
YAML
}
  ...
  metadata {
    ...
    annotations = {
      ...
      "cloud.google.com/backend-config" = jsonencode({
        default = "fsp-test-generation-backend"
      })
    }
