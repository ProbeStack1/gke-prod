resource "kubernetes_deployment_v1" "fq_setting_mgmt_svc" {
  metadata {
    name      = "fq-setting-mgmt-svc"
    namespace = "forgeq-prod"

    labels = {
      app = "fq-setting-mgmt-svc"
    }
  }
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name  = "fq-setting-mgmt-svc"
          image = var.fq_setting_mgmt_svc_image
          ...
          readiness_probe {
            http_get {
              path = "/api/v1/settings/actuator/health"
              port = 8080
            }
...
resource "kubectl_manifest" "fq_setting_mgmt_backend" {
  yaml_body = <<YAML
apiVersion: cloud.google.com/v1
kind: BackendConfig
metadata:
  name: fq-setting-mgmt-backend
  namespace: forgeq-prod
spec:
  healthCheck:
    requestPath: /api/v1/settings/actuator/health
    port: 8080
    type: HTTP
YAML
}

resource "kubernetes_service_v1" "fq_setting_mgmt_svc" {
  metadata {
    name      = "fq-setting-mgmt-svc"
    namespace = "forgeq-prod"
    ...
    annotations = {
      ...
      "cloud.google.com/backend-config" = jsonencode({
        default = "fq-setting-mgmt-backend"
      })
    }
