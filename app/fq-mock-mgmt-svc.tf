resource "kubernetes_deployment_v1" "fq_mock_mgmt_svc" {
  metadata {
    name      = "fq-mock-mgmt-svc"
    namespace = "forgeq-prod"

    labels = {
      app = "fq-mock-mgmt-svc"
    }
  }
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name  = "fq-mock-mgmt-svc"
          image = var.fq_mock_mgmt_svc_image
          ...
          readiness_probe {
            http_get {
              path = "/api/v1/mocks/actuator/health"
              port = 8080
            }
...
resource "kubectl_manifest" "fq_mock_mgmt_backend" {
  yaml_body = <<YAML
apiVersion: cloud.google.com/v1
kind: BackendConfig
metadata:
  name: fq-mock-mgmt-backend
  namespace: forgeq-prod
spec:
  healthCheck:
    requestPath: /api/v1/mocks/actuator/health
    port: 8080
    type: HTTP
YAML
}

resource "kubernetes_service_v1" "fq_mock_mgmt_svc" {
  metadata {
    name      = "fq-mock-mgmt-svc"
    namespace = "forgeq-prod"
