resource "kubernetes_deployment_v1" "fq_testfile_mgmt_svc" {
  metadata {
    name      = "fq-testfile-mgmt-svc"
    namespace = "forgeq-prod"

    labels = {
      app = "fq-testfile-mgmt-svc"
    }
  }
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name  = "fq-testfile-mgmt-svc"
          image = var.fq_testfile_mgmt_svc_image
          ...
          readiness_probe {
            http_get {
              path = "/api/v1/testfiles/actuator/health"
              port = 8080
            }
...
resource "kubectl_manifest" "fq_testfile_mgmt_backend" {
  yaml_body = <<YAML
apiVersion: cloud.google.com/v1
kind: BackendConfig
metadata:
  name: fq-testfile-mgmt-backend
  namespace: forgeq-prod
spec:
  healthCheck:
    requestPath: /api/v1/testfiles/actuator/health
    port: 8080
    type: HTTP
YAML
}

resource "kubernetes_service_v1" "fq_testfile_mgmt_svc" {
  metadata {
    name      = "fq-testfile-mgmt-svc"
    namespace = "forgeq-prod"
    ...
    annotations = {
      ...
      "cloud.google.com/backend-config" = jsonencode({
        default = "fq-testfile-mgmt-backend"
      })
    }
