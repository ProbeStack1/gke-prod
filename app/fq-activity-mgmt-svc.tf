resource "kubernetes_deployment_v1" "fq_activity_mgmt_svc" {
  metadata {
    name      = "fq-activity-mgmt-svc"
    namespace = "forgeq-dev"
    labels = {
      app = "fq-activity-mgmt-svc"
    }
  }
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name  = "fq-activity-mgmt-svc"
          image = var.fq_activity_mgmt_svc_image
          ...
          readiness_probe {
            http_get {
              path = "/api/v1/activity/actuator/health"
              port = 8080
            }
...
resource "kubectl_manifest" "fq_activity_mgmt_backend" {
  yaml_body = <<YAML
apiVersion: cloud.google.com/v1
kind: BackendConfig
metadata:
  name: fq-activity-mgmt-backend
  namespace: forgeq-dev
spec:
  healthCheck:
    requestPath: /api/v1/activity/actuator/health
    port: 8080
    type: HTTP
YAML
}

resource "kubernetes_service_v1" "fq_activity_mgmt_svc" {
  metadata {
    name      = "fq-activity-mgmt-svc"
    namespace = "forgeq-dev"
